...
public class DumpManager {
	...
	private String collection2CSV(Collection<Ticket> collection) {
		try {
			...
			for (var e : collection) {
				csvWriter.writeNext(Ticket.toArray(e));
			}
	 ...
	 */
	public void writeCollection(Collection<Ticket> collection, Collection<Ticket> collectionDie, ArrayDeque<String> logStack) {
	 ...
	 * @return коллекция
	 */
	private LinkedList<Ticket> CSV2collection(String s) {
		try {
			StringReader sr = new StringReader(s);
			CSVReader csvReader = new CSVReader(sr, ';');
			LinkedList<Ticket> ds = new LinkedList<Ticket>();
			String[] record = null;
			while ((record = csvReader.readNext()) != null) {
				Ticket d = Ticket.fromArray(record);
				if (d.validate())
					ds.add(d);
				else
	 ...
	 */
	public void readCollection(Collection<Ticket> collection, Collection<Ticket> collectionDie, ArrayDeque<String> logStack) {
		try {
		...
		collection = new LinkedList<Ticket>();
	}
